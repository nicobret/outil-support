...
export default () => {
  ...
  const createFolder = async (folder) => {
    try {
      ...
      const { ok } = await API.post({ path: "/folder", body: { folder } });
      if (ok) {
  ...
  const updateFolder = async (folder) => {
    try {
      const { ok } = await API.put({ path: `/folder/${folder._id}`, body: { name: folder.name, abbreviation: folder?.abbreviation } });
      if (ok) {
  ...
  const onSortEnd = async (e) => {
    if (e.newIndex !== -1) {
      var newFolders = arrayMoveImmutable(folders, e.newIndex, e.oldIndex);
      for (let i = 0; i < newFolders.length; i++) {
        newFolders[i].folderIndex = i;
      }
      const { ok } = await API.post({ path: "/folder/reindex", body: { folders: newFolders } });
      if (ok) {
