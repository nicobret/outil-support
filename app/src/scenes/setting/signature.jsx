...
import { classNames, translateRole } from "../../utils";
...
export default function Shortcut() {
  ...
  const update = async (input) => {
    try {
      const { ok, data } = await API.post({ path: "/shortcut/search", body: { q: input, filter, isSignature: true } });
  ...
  const updateShortcut = async (shortcut) => {
    try {
      const { ok } = await API.put({ path: `/shortcut/${shortcut._id}`, body: shortcut });
      if (ok) {
        await update();
        ...
        >
          <Popover.Panel className="absolute z-10 mt-2 flex w-full min-w-[224px] origin-top-left flex-col rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5">
            {Object.values(translateRole).map((role) => (
              <Checkbox name={role} state={selectedContactGroup.includes(role)} setState={(v) => handleChangeState(role, v)} />
            ))}
          </Popover.Panel>
        </Transition>
      </Popover>
