...
import { translateRole, translateSource, translateState, translateVentilationOperator, translateVentilationField, translateVentilationFieldAction } from "../../utils";
import { departmentList, regionList } from "../../utils/region-and-departments.utils.js";
...
export default function VentilationAutomation() {
  ...
  const updateVentilation = async (ventilation) => {
    try {
      const { ok } = await API.put({ path: `/ventilation/${ventilation._id}`, body: ventilation });
      if (ok) {
        await update();
...
const ModalContent = ({ open, setOpen, ventilation, isUpdate, agents, tags, folders }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const { ok } = !isUpdate
        ? await API.post({ path: "/ventilation", body: { name, description, conditionsEt: conditionsEt, conditionsOu: conditionsOu, actions, active } })
        : await API.put({
            path: `/ventilation/${ventilation._id}`,
            body: { name, description, conditionsEt: conditionsEt, conditionsOu: conditionsOu, actions, active },
          });
      if (ok) {
  ...
  useEffect(() => {
    console.log("c", conditionsEt, "cOu", conditionsOu, "a", actions);
  }, [conditionsEt, conditionsOu, actions]);
