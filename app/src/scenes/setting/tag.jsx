...
export default function Label() {
  ...
  const update = async (q) => {
    try {
      const { ok, data } = await API.get({ path: `/tag/search`, query: { q: q || "" } });
  ...
  const getAggregations = async (tags) => {
    try {
      const { ok, data } = await API.post({ path: "ticket/stats/tags", body: { period: 1000 } });
      setTags(
        tags.map((tag) => {
  ...
  const updateTag = async (tag) => {
    try {
      const { ok } = await API.put({ path: `/tag/${tag._id}`, body: tag });
      if (ok) await update();
