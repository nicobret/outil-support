...
export default function Shortcut() {
  ...
  const update = async (input) => {
    try {
      const { ok, data } = await API.post({ path: "/kb-search", body: { q: input, filter } });
  ...
  const exportData = async () => {
    try {
      ...
      console.log(e);
      toast.error("Erreur !", translate(e.code));
    }
  };
    ...
    <div>
        ...
        >
          <Popover.Panel className="absolute z-10 mt-2 flex w-full min-w-[224px] origin-top-left flex-col rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5">
            {Object.keys(translateRoleBDC).map((role) => (
              <Checkbox name={translateRoleBDC[role]} state={selectedContactGroup.includes(role)} setState={(v) => handleChangeState(role, v)} />
            ))}
          </Popover.Panel>
        </Transition>
      </Popover>
      <div className=" mb-2 grid grid-cols-1 gap-1">
        {selectedContactGroup.map((c) => (
          <span className=" mt-1 rounded-xl bg-purple-100 px-1 text-center font-medium text-purple-800">{translateRoleBDC[c]}</span>
        ))}
      </div>
    </div>
  );
