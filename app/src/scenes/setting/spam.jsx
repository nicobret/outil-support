...
export default function Spam() {
  ...
  async function addSpam() {
    try {
      const response = await API.put({ path: `/organisation/${organisation._id}`, body: { spamEmails: [...spams, newSpam] } });
      if (response.ok) {
  ...
  async function delSpam(deletedSpam) {
    try {
      const response = await API.put({ path: `/organisation/${organisation._id}`, body: { spamEmails: spams.filter((spam) => spam !== deletedSpam) } });
      if (response.ok) {
        dispatch(setOrganisation(response.data));
        toast.success("Ok");
      }
