...
export default function SideBar({ aggregations, filter, update }) {
  ...
  async function getFolder() {
    try {
      ...
      toast.error(e, "Erreur lors de la récupération des tags");
    }
  }
...
const Item = ({ onDrop = () => {}, onDragOver = () => {}, total, name, selected, onClick, onSortEnd, folderIndex, isFolder, id }) => {
  const [dragging, setDragging] = useState(false);
  return (
    <div
      onDragOver={(ev) => {
        setDragging(true), onDragOver(ev);
      }}
      onDrop={(ev) => {
        onDrop(ev, id);
        setDragging(false);
    ...
    >
      <button
        type="button"
        className={`${dragging === true ? "bg-gray-500" : "bg-white"} ${
          isFolder ? "grid grid-cols-[20px_180px_20px] pl-7" : "flex justify-between pl-14"
        }  w-full items-center  gap-2 py-3 transition-colors hover:bg-gray-50`}
        onClick={onClick}
      >
        {isFolder && (
          <div>
            <div>
              <HiChevronUp
                className="text-xl"
                onClick={(e) => {
                  onSortEnd({ oldIndex: folderIndex, newIndex: folderIndex - 1 });
                  e.stopPropagation();
                }}
              />
            </div>
        ...
        <span className={classNames(selected ? "font-bold text-accent-color" : "font-medium text-gray-900", "text-left text-base")}>{name}</span>
        {total !== 0 && <span className="flex-none rounded-full bg-[#FFDBD9]  py-0.5 text-xs font-medium text-[#C93D38]">{total}</span>}
      </button>
    </div>
  );
};
