...
const Thread = ({
  ...
  const onUploadImage = async (formatedMessage) => {
    try {
      const { ok, code } = await API.uploadFile(`/message/sendEmailFile/${ticket._id}`, files, { message: formatedMessage, copyRecipient, dest, messageHistory });
      if (!ok) return false;
      setReloadKey((k) => k + 1);
      setOpen(false);
      updateMessages();
  ...
  const handleAddDraftMessage = async () => {
    ...
    try {
      const { ok, code } = await API.put({
        path: `/ticket/${ticket._id}`,
        body: { messageDraft: messageWithoutHTMLTags ? removeLineBreakFromStartAndEnd(messageWithoutSigniture) : "", ticketId: ticket._id },
      });
      if (!ok) return toast.error(`Une erreur est survenue : ${code}`);
          ...
          {viewingAgents.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              {viewingAgents.map((agent) => (
                <div data-tip={`${agent.firstName} ${agent.lastName} - ${TRANSLATE_ROLE[agent.role]}`}>
