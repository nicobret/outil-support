...
export default function ChatBox({
  ...
  const downloadFileFromS3 = async (file) => {
    try {
      const { ok, data } = await API.post({ path: `/message/s3file`, body: { path: file.path } });
        ...
        {files.map((file) => {
          return (
            <div key={file._id} className={`relative ${classNames(sender ? "" : "ml-[52px]", "mt-2.5 flex items-center text-[22px] text-grey-text")}`}>
              ...
              <a href={publicUrl} target="_blank" className="hidden text-sm text-gray-500 underline" ref={linkRef}>
                {file.name}
              </a>
              <span
