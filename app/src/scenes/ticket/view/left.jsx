...
export default ({ ticket, setTicket, tags, setTags, agents }) => {
  ...
  const handleChangeRole = async (contactGroup) => {
    try {
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: { contactGroup } });
...
const Notes = ({ ticket, setTicket, agents }) => {
  ...
  async function addNote() {
    ...
    note.content = str;
    const { data } = await API.put({ path: `/ticket/${ticket._id}`, body: { notes: [...(ticket.notes || []), note] } });
    setTicket(data.ticket);
    setStr("");
  ...
  async function deleteNote(id) {
    if (window.confirm("Voulez-vous vraiment supprimer cette note ? Cette opération est définitive")) {
      const { data } = await API.put({ path: `/ticket/${ticket._id}`, body: { notes: ticket.notes.filter((note) => note._id !== id) } });
      setTicket(data.ticket);
      ...
      {open && (
        <div className={`${!editedNote?._id && "max-h-64"} min-h-0 overflow-y-auto`} disabled={user.role !== "DG"}>
          {ticket.notes?.map((e, index) => {
            ...
            return (
              <div className={`mb-[22px] text-xs text-gray-500 ${index === ticket.notes.length - 1 && "rounded-lg "}`} key={e._id}>
...
const Role = ({ attributes }) => {
  return (
    <div className="flex flex-col text-left text-sm text-gray-700 transition-colors hover:bg-gray-50">
      {attributes?.map((e) => {
        if (e?.name === "role") {
          return translateRole[e?.value];
        }
      })}
    </div>
  );
...
const TagsEditor = ({ name, tags, ticket, setTicket, setTags, user }) => {
  ...
  const addTag = async (tag) => {
    try {
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: { tags: [...new Set([...tags, tag])] } });
      if (!ok) return toast.error(e, "Erreur lors de l'ajout du tag");
      setTicket(data.ticket);
      setTags(data.tags);
  ...
  const removeTag = async (tag) => {
    try {
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: { tags: tags.filter((t) => t !== tag) } });
      if (ok) {
        setTicket(data.ticket);
        setTags(data.tags);
  ...
  if (attribute?.format === "link" || attribute.value?.includes("https://"))
    return (
      <div className="flex mb-2">
        <span className="text-xl text-[#C4C4C4]">{emoji}</span>
        <a href={attribute.value} target="_blank" className="pl-1 text-sm font-medium text-gray-500 underline ">
          {attribute.name}
        </a>
      </div>
    );
