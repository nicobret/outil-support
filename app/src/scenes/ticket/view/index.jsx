...
import { HiOutlineChevronLeft, HiChevronDoubleDown, HiOutlineDesktopComputer, HiOutlineTicket, HiOutlineMail, HiOutlineCalendar } from "react-icons/hi";
...
import { getDocumentTitle, getStatusColor, sourceToIcon, translateFeedback, translateRole, translateState, TRANSLATE_ROLE, formatTicketDate } from "../../../utils";
...
const Header = ({ ticket, setTicket, deleteModalOpen, setDeleteModalOpen, agents, referentsDepartment, referentsRegion, user }) => {
  ...
  const handleChangeStatus = async (status) => {
    try {
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: { status } });
  ...
  const handleChangeCanal = async (canal) => {
    try {
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: { canal } });
  ...
  const handleChangeFeedback = async (feedback) => {
    try {
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: { feedback } });
        ...
        <div>
          <div className="flex">
            ...
            <div className="flex mt-1 ml-2">
              ...
              <span className="mb-1 text-sm text-gray-900 ">{lastActivity}</span>
            </div>
          </div>
