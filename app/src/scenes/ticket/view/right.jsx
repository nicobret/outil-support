...
export default ({ ticket, agents }) => {
  ...
  const handleAttribute = async (ticket, attribute) => {
    try {
      ...
      const { ok, data } = await API.put({ path: `/ticket/${ticket._id}`, body: ticket });
      if (ok) {
...
const TagItem = ({ ticket }) => (
  <div className="flex items-center hover:cursor-pointer" onClick={() => window.open(`${appURL}/ticket/${ticket._id}`, "_blank")}>
