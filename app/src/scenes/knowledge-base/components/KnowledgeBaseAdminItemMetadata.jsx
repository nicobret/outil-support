...
const KnowledgeBaseAdminItemMetadata = ({ visible }) => {
  const [isHistoricSectionOpen, setHistoricSectionOpen] = useState(false);
  ...
  const { flattenedKB, knowledgeBaseTree, setFlattenedKB, refreshKB, loadPatches, patches, patchesLoading } = useContext(KnowledgeBaseContext);
  ...
  const onSubmit = async (event) => {
    ...
    const response = await API.put({ path: `/knowledge-base/${item._id}`, body });
    setIsSubmitting(false);
    await loadPatches(item._id);
  ...
  const onUploadImage = async (event) => {
    ...
    const response = await API.put({ path: `/knowledge-base/${item._id}`, body: { imageSrc: imageRes.data } });
    setIsSettingImg(false);
    if (response.code) return toast.error(response.code);
    refreshKB();
  ...
  const onDeleteImage = async () => {
    if (window.confirm("Voulez-vous vraiment supprimer cet image ? Cette opération est définitive")) {
      ...
      const response = await API.put({ path: `/knowledge-base/${item._id}`, body: { imageSrc: null } });
  ...
  const onSelectIcon = async (icon) => {
    ...
    const response = await API.put({ path: `/knowledge-base/${item._id}`, body: { icon } });
      ...
      <ResizablePanel className={`z-10 flex shrink-0 grow-0 overflow-hidden border-l-2 ${visible ? "w-80" : "hidden w-0"}`} name="admin-knowledge-base-metadata" position="right">
        <div className="overflow-auto">
          <div className="px-4 pt-6 pb-2">
            <button
              className="text-grey-700 flex w-full items-center justify-between py-3 text-base font-medium uppercase"
              onClick={() => setHistoricSectionOpen(!isHistoricSectionOpen)}
            >
              <div className="flex items-center">
                Historique
                <HiOutlineClock className="ml-2" />
              </div>

              {isHistoricSectionOpen ? <HiChevronDown className="ml-2" /> : <HiChevronRight className="ml-2" />}
            </button>
            <div className="mb-4 text-sm">
              {isHistoricSectionOpen && patchesLoading && <Loader size={20} />}
              {isHistoricSectionOpen && !patchesLoading && patches.length === 0 && <div>Pas d'historique récente</div>}
              {isHistoricSectionOpen && (
                <div>
                  {patches.map(({ author, date, action, email }, i) => (
                    <div key={i} className="flex border-t border-gray-300 py-2">
                      <Avatar email={email} color="bg-[#C93D38]" className="mr-2 shrink-0 self-center" />
                      <div className="w-full">
                        <div className="flex justify-between">
                          <span className="mr-2 flex-1 font-medium text-black-dark">{author}</span>
                          <span className="text-grey-text">{new Date(date).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })}</span>
                        </div>
                        <span className="whitespace-pre-line text-grey-text">{action}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <form onSubmit={onSubmit} className="flex h-full w-full shrink-0 grow-0 flex-col items-start px-4 pb-6" key={item._id}>
            <p className="mb-5">
