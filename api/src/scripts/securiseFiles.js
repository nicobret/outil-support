...
const { PUBLIC_BUCKET_NAME_SUPPORT_OLD, PUBLIC_BUCKET_NAME_SUPPORT } = require("../config");
...
async function process(message, count, total) {
  ...
  for (const attachment of message.attachments) {
    // download file
    try {
      const downloadedFile = await getFile(attachment.path, PUBLIC_BUCKET_NAME_SUPPORT_OLD);
